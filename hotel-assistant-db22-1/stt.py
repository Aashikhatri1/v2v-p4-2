...
class Transcriber:
        ...
        while attempt_count < self.max_reconnect_attempts:
            try:
                async with websockets.connect(
                        "wss://api.deepgram.com/v1/listen?punctuate=true&encoding=linear16&sample_rate=16000&endpointing=300",
    ...
    #             # if full_transcript.strip() and res.get("speech_final"):
    #             #     print('speech_final')
    #             #     # If the message is marked as speech_final, return the full transcript
    #             #     if full_transcript.strip() and self.on_transcript:
    #             #         print('full_transcript',full_transcript)
    ...
    # # Initialize and start the audio stream
    #     p = pyaudio.PyAudio()
    #     self.stream = p.open(format=FORMAT, channels=CHANNELS, rate=RATE, input=True, frames_per_buffer=CHUNK, stream_callback=self.mic_callback)
    #     self.stream.start_stream()

    #     # Start the main transcription task and the periodic call end check
    #     transcription_task = asyncio.create_task(self.connect_and_transcribe(key))
    #     call_end_check_task = asyncio.create_task(self.check_call_end_periodically())

    #     # Wait for either task to complete
    #     await asyncio.gather(transcription_task, call_end_check_task)

    #     # Cleanup after tasks completion or cancellation
    #     if self.stream.is_active():
    #         self.stream.stop_stream()
    #     self.stream.close()
    #     p.terminate()